...
import { generateEmergencyProtocol } from '../services/geminiService';
...
const EmergencyProtocol: React.FC<EmergencyProtocolProps> = ({ profile }) => {
    ...
    const handleGenerate = async () => {
        ...
        setProtocol('');
        try {
            const result = await generateEmergencyProtocol(profile);
            setProtocol(result);
        } catch (err) {
    ...
    const handlePrint = () => {
        ...
        if (printWindow) {
            printWindow.document.write('<html><head><title>Emergency Protocol</title>');
            ...
            printWindow.document.write('<h1>Emergency Protocol</h1>');
            printWindow.document.write(`<pre>${protocol}</pre>`);
        ...
        <div className="w-full max-w-3xl mx-auto">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 p-8">
                <div className="flex justify-between items-start mb-4">
                    <div>
                        <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-2">Emergency Protocol Generator</h3>
                        <p className="text-slate-600 dark:text-slate-300">Create a personalized crisis plan based on your profile.</p>
                    </div>
                     {protocol && (
                ...
                {!protocol && !isLoading && !error && (
                    <div className="text-center py-8">
                        ...
                        {profileIsEmpty && <p className="text-sm text-slate-500 dark:text-slate-400 mt-3">Please fill out your Autism Profile to generate a protocol.</p>}
                    </div>
                )}
                
                {isLoading && <div className="text-center text-slate-500 dark:text-slate-400">Generating your protocol...</div>}
                ...
                {protocol && (
                    <div className="mt-6 p-6 bg-purple-50 dark:bg-purple-900/50 border border-purple-200 dark:border-purple-700 rounded-lg animate-fade-in">
                         <h4 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-2">Your Personalized Protocol:</h4>
                        <pre className="text-slate-700 dark:text-slate-200 whitespace-pre-wrap font-sans">{protocol}</pre>
                    </div>
                )}
            </div>
        </div>
